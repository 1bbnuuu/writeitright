...
const LevelOne = () => {
    ...
    const [leftItemsState, setLeftItemsState] = useState(
        () => getItemPositions(currentStage).leftItems,
    );
    const [rightItemsState, setRightItemsState] = useState(
        () => getItemPositions(currentStage).rightItems,
    );
        ...
        if (rightItem && !rightItem.matched) {
            ...
            if (isCorrect) {
                ...
                setLeftItemsState((prev) =>
                    prev.map((item) =>
                        item.id === currentLine.startItem.id
                            ? { ...item, matched: true }
                            : item,
                    ),
                );
                ...
                const totalMatched =
                    leftItemsState.filter((item) => item.matched).length + 1;
            ...
            <div className="flex-1 flex justify-center items-center p-4">
                <div className="relative z-10 bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl overflow-hidden">
                    ...
                    {leftItemsState.map((item) => (
                        ...
                        >
                            <div
                                className={`bg-white rounded-full p-2 border-4 ${
                                    item.matched
                                        ? "border-green-300"
                                        : "border-orange-300"
                                } shadow-lg`}
